...
Supports both images and videos (sequences).

Usage:
    python sft_train.py --data_path ./gastrohun_llm_en_images_train.jsonl \\
        --image_dir /path/to/media --runname oral_sft_v1 --mode 1
"""
...
warnings.filterwarnings(
    "ignore",
    message=r".*Asked to sample `fps` frames per second but no video metadata was provided.*",
)
...
class OralSFTDataset(torch_data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # CoT-SFT: prefer cot target when available, fallback to original gt text.
        target_text = sample.get("gt_text_cot") or sample["gt_text"]
...
def main():
    ...
    if not train_samples:
        ...
        sys.exit(1)
    cot_count = sum(1 for s in train_samples if s.get("gt_text_cot"))
    print(f"CoT targets available: {cot_count}/{len(train_samples)} (empty entries fallback to gt_text)")
