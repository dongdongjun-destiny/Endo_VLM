...
SYSTEM_PROMPT = """You are an expert endoscopic assistant. Analyze the oral description and gastroscopy image.

You must classify into exactly one SSS label from:
...
# ==============================================================================
# GPU GUARD (protect running exvla job from other GPU training)
# ==============================================================================

GPU_GUARD_CONFIG = {
    "enabled": True,
    # Conservative default: only kill when cmdline matches training_cmd_hints.
    "require_cmd_match_for_kill": True,
    "watchdog": True,
    "watchdog_poll_sec": 10.0,
    "min_kill_mem_mib": 8192,  # 8 GiB
    "min_training_cmd_mem_mib": 512,
    # External training only; excludes sft_train.py / rft_grpo_* (protected via process tree).
    "training_cmd_hints": [
        "finetune", "openpi", "compute_norm_stats",
...
RFT_CONFIG = {
    ...
    "wandb_project": "endovla_oral",
    "wandb_run_group": "rft",
    # GRPO rollout only (not INSTRUCT_GENERATION_CONFIG used for eval/inference).
...
def get_grpo_train_config(modality: Optional[str] = None, reward_style: Optional[str] = None) -> dict:
    """RFT hyperparameters + reward flags for GRPO (single dict for train_rft).

    Does not override RFT_CONFIG hyperparameters. Only sets reward_style when omitted:
      video / default -> short  (multimodal_train short CoT targets)
      image           -> cot     (images_train_cot1 long CoT targets)
    """
    cfg = {**RFT_CONFIG, **REWARD_CONFIG}
    if reward_style:
        cfg["reward_style"] = reward_style
    elif modality == "image":
        cfg["reward_style"] = "cot"
    else:
        cfg["reward_style"] = "short"
    return cfg
...
def normalize_training_sample(item: dict, image_dir: str = "") -> Optional[dict]:
    ...
    gt_text_cot = item.get("gt_text_cot", "")
    if not gt_text:
    ...
    return {
        ...
        "doctor_score": item.get("doctor_score", None),
...
def build_train_video_content(video_path: str) -> dict:
    ...
    Use fixed-frame sampling (nframes) instead of fps-based sampling to avoid
    warnings when backend video metadata is unavailable.
    """
    return {
        "type": "video",
        "video": video_path,
        "nframes": TRAIN_VIDEO_MAX_FRAMES,
...
def setup_processor_image_size(processor):
    ...
    if hasattr(processor, "video_processor"):
        ...
        if hasattr(vp, "nframes"):
            vp.nframes = TRAIN_VIDEO_MAX_FRAMES
        if hasattr(vp, "size"):
        ...
        # Disable fps-based sampling path to avoid metadata-dependent warnings.
        if hasattr(vp, "fps"):
            vp.fps = None
...
def validate_config():
    ...
    print(f"RFT: batch={RFT_CONFIG['batch_size']}, epochs={RFT_CONFIG['num_epochs']}, "
          ...
          f"sampling T={RFT_CONFIG['grpo_temperature']} top_p={RFT_CONFIG['grpo_top_p']} "
          f"top_k={RFT_CONFIG['grpo_top_k']}")
