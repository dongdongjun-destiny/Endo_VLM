...
def repartition_records(
    ...
    for split_name, split_rows in split_map.items():
        for i, row in enumerate(split_rows):
            row["split"] = split_name.capitalize()
            row["id"] = f"{id_prefix}_{split_name}_{i:06d}"
    return split_map
...
def build_images_jsonl() -> Dict[str, List[Dict[str, object]]]:
    ...
    for split_name, split_df in split_dfs.items():
        all_records.extend(build_image_samples(split_df, split_name))

    out = repartition_records(all_records, "img")
...
def build_sequences_jsonl() -> Dict[str, List[Dict[str, object]]]:
    ...
    all_records: List[Dict[str, object]] = []
    for split_name, split_df in split_dfs.items():
        all_records.extend(build_seq_samples(split_df, split_name))

    out = repartition_records(all_records, "seq")
    for split_name, records in out.items():
...
def build_multimodal_jsonl(
    ...
    for split_name in ["train", "val", "test"]:
        all_records.extend(image_records.get(split_name, []))
        all_records.extend(seq_records.get(split_name, []))

    merged_splits = repartition_records(all_records, "mm")
    for split_name, merged in merged_splits.items():
